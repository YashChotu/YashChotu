...
// Animate section titles and main content only when in view
const sectionTitleObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            const title = entry.target.querySelector('.section-title');
            if (title) title.classList.add('animated');
            const mainContent = entry.target.querySelector('.about-content, .education-content, .skills-grid, .projects-grid, .contact-content');
...
document.querySelectorAll('section').forEach(section => {
    sectionTitleObserver.observe(section);
});
...
// Enhanced Scroll Animations for Achievement Cards
...
// Why Work With Me cards animation
